  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white px-6 py-10">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <section className="bg-slate-800 p-6 rounded-lg">
          ...
          <ul className="mt-4 space-y-3">
            {legalData.agreements.map((doc, idx) => (
              <li
                key={idx}
                className="flex items-center justify-between bg-slate-700 px-4 py-3 rounded text-sm"
              >
                <span>{doc.name}</span>
                <a
                  href={doc.url}
        ...
        <section className="bg-slate-800 p-6 rounded-lg border border-yellow-400/30">
          <h2 className="text-lg font-semibold flex items-center gap-2 text-yellow-300">
